...
class EnumNumberSerializer<T : Enum<T>>(
    ...
    override fun deserialize(decoder: Decoder): T {
        return serializableClass.enumMembers().let {
            ...
            if (it.size < index || index < 0) {
                throw IllegalArgumentException("unknown index $index for enum ${serializableClass.enumClassName()}")
            }
            it[index]
        }
    }
    ...
    override fun serialize(encoder: Encoder, obj: T) {
        if (obj !in serializableClass.enumMembers()) {
            throw IllegalArgumentException("unknown object $obj for enum ${serializableClass.enumClassName()}")
        }
